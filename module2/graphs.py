...
def generate_graph(features1, features2, document_path1, document_path2, threshold_value):
    ...
    for i in range(len(features)):
        for j in range(i + 1, len(features)):
            ...
            if link_score > threshold_value:
                print(wn.synsets(features[i])[0].path_similarity(wn.synsets(features[j])[0]))
                graph.add_edge(features[i], features[j], weight=(link_score+wn.synsets(features[i])[0].path_similarity(wn.synsets(features[j])[0])))
    ...
    plt.show()
    sT = nx.contracted_nodes(graph,"device", "openings")
    #sT = steiner_tree(graph, ["passage", "openings", "network"], weight="weight")
    nx.draw_networkx(sT, nx.spring_layout(sT))
    # nx.draw_circular(graph)
    plt.show()
...
if __name__ == '__main__':
    feature1 = ["regulator","anti-Scaling", "passage", "", "network", "invention", "interior"]
    feature2 = ["regulator", "housing", "device", "anti-Scaling", "passage", "openings", "network", "invention", "interior"]
    document1 = "./Document_1.pdf"
    document2 = "./Document_2.pdf"
    generate_graph(feature1, feature2, document1, document2, 0.2)
